...
async function callAI(prompt, systemMsg) {
  try {
    const system = systemMsg || 'You are a helpful school tutor. Be clear and friendly. Use plain text only, no markdown, no asterisks, no bullet symbols.';
    ...
    return d.candidates?.[0]?.content?.parts?.[0]?.text || 'No response received. Try again.';
  } catch (e) {
...
async function askAI() {
  ...
  showLoading(r);
  const ans = await callAI(q, 'You are a friendly school tutor. Answer clearly and helpfully. Use plain text only — no markdown, no asterisks, no bullet point symbols. Use numbered lists or plain dashes if needed.');
  r.textContent = ans;
...
function calcGrade() {
  ...
  bar.style.width = pct + '%';
  bar.className = 'usage-fill' + (pct >= 70 ? '' : pct >= 60 ? ' warning' : ' danger');
}
...
function countWords() {
  ...
  document.getElementById('wc-result').textContent = `${words} words · ${chars} characters · ~${readMins} min read`;
}
...
async function getEssayFeedback() {
  ...
  showLoading(o);
  const ans = await callAI('Give constructive feedback on this text. Be specific. Point out 2-3 strengths and 2-3 things to improve:\n\n' + t, 'You are a writing coach for students. Give friendly, honest, practical feedback. Plain text only, no markdown or asterisks.');
...
async function makeFlashcards() {
  ...
  if (!allowed) { showBlocked(o); return; }
  showLoading(o);
  const ans = await callAI('Create 5 flashcard Q&A pairs from this topic or notes. Format each exactly like:\nQ: [question]\nA: [answer]\n\nTopic/notes:\n' + t, 'You make flashcards for students. Clear, memorable Q&A pairs. Plain text only.');
  o.textContent = ans; o.style.display = 'block';
  ...
  if (timerRunning) {
    ...
    timerInterval = setInterval(() => {
      if (timerSecs > 0) {
        ...
        clearInterval(timerInterval); timerRunning = false;
        document.getElementById('start-btn').textContent = 'Start';
        document.getElementById('timer-label').textContent = "time's up! great work 🎉";
        unlockBadge('badge-timer');
      }
    }, 1000);
  }
}
...
function resetTimer() {
  clearInterval(timerInterval); timerRunning = false; timerSecs = 25 * 60;
  document.getElementById('timer-display').textContent = '25:00';
  document.getElementById('start-btn').textContent = 'Start';
  document.getElementById('timer-label').textContent = 'pomodoro — 25 min focus';
}
...
async function getJoke() {
  ...
  const allowed = await checkAndIncrementUsage();
  if (!allowed) { showBlocked(o); return; }
  const ans = await callAI('Tell me one funny school-related joke. Just the joke, no intro or explanation.', 'You tell short funny school jokes. One joke only. Plain text.');
  o.textContent = ans;
...
async function getMotivation() {
  ...
  const allowed = await checkAndIncrementUsage();
  if (!allowed) { showBlocked(o); return; }
  const ans = await callAI('Give me one short motivational message for a student who is studying. Make it genuine and not cliché. Max 2 sentences.', 'You give short real motivational messages to students. Plain text, no quotes or asterisks.');
  o.textContent = ans;
}
...
async function getStudyFact() {
  ...
  const allowed = await checkAndIncrementUsage();
  if (!allowed) { showBlocked(o); return; }
  const ans = await callAI('Give me one surprising or interesting fact about learning, memory, the brain, or a school subject. Keep it to 2-3 sentences.', 'You share interesting educational facts. Plain text only. Be specific and genuinely interesting.');
  o.textContent = ans;
}
